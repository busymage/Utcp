...
{
    return impl_->sock->bind(port);
}

int Socket::connect(uint32_t addr, uint16_t port)
{
    return impl_->sock->connect(addr, port);
}
...
{
    return impl_->sock->send(buffer);
}

int Socket::recv(std::vector<uint8_t> &buffer)
{
    return impl_->sock->recv(buffer);
}
